      ...
      output: {
        ...
        createdUsers: z =>
          z
            ...
            .optional()
            .describe('List of created user accounts (parsed from output)'),
        userSummary: z =>
          z
            .string()
            .optional()
            .describe('Formatted summary of created users'),
      },
    },
    async handler(ctx) {
      const {
      ...
      try {
        ...
        // Parse user creation output for workshop scripts
        let createdUsers: Array<{username: string, password: string, sshCommand: string}> = [];
        let userSummary = '';
        
        if (scriptArgs?.USER_PREFIX && scriptArgs?.USER_COUNT && scriptArgs?.DEFAULT_PASSWORD) {
          const userPrefix = scriptArgs.USER_PREFIX as string;
          const userCount = parseInt(scriptArgs.USER_COUNT as string, 10);
          const defaultPassword = scriptArgs.DEFAULT_PASSWORD as string;
          
          // Generate user list based on script parameters
          for (let i = 1; i <= userCount; i++) {
            const username = `${userPrefix}${i.toString().padStart(2, '0')}`;
            const sshCmd = `ssh ${username}@${host}`;
          ...
          const userList = createdUsers.map(u => 
            `• ${u.username} (Password: ${u.password}) - ${u.sshCommand}`
          ).join('\n');
          
          userSummary = `Created ${userCount} user accounts:\n${userList}`;
          
          ctx.logger.info(`Parsed user creation data: ${userCount} users with prefix ${userPrefix}`);
        }
        ...
        if (createdUsers.length > 0) {
          ctx.output('createdUsers', createdUsers);
          ctx.output('userSummary', userSummary);
        }
